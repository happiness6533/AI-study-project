...
#==============================================================================
# To keep experimenting with Keras and TensorFlow,
# we will implement another custom callback.
# Instead of waiting the end of the training to visualize some predicted images,
# would it be not better to plot some result grids after each epoch for instance?
# Even better: we could reuse the same input/target images each time,
# so we can compare the predicted versions from one epoch to another!
# The following custom callback does exactly that.
# At the end of each epoch,
# it feeds the model with some pre-selected images,
# then use matplotlib to draw a result grid.
# It then converts the pyplot.
# Figure into a tensorflow.
# Summary which can be saved by TensorFlow an read by Tensorboard for display
