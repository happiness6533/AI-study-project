...
class ConvWithBatchNorm(tf.keras.layers.Conv2D):
    ...
    def __init__(self, activation='relu', name='convbn', **kwargs):
        ...
        self.adj_w = tf.Variable(initial_value=tf.random.normal(shape=(input_dim, units), mean=0., stddev=1.),
                             trainable=True)
    ...
    def call(self, inputs, training=None):
        ...
        # filter 1개로 해석된 결과를 하나의 노드로 보면 현재 filter 개수만큼의 노드가 있다고 가정한다
        # 1. n*n*filter => n^2 * filter => 트랜스포즈 => filter개수 * n^2
        # 2. filter개수 * n^2 곱하기 n^2 * n^2 => filter개수 * n^2 행렬 나옴
        # 3. filter 개수 * filter개수 행렬을 관계 그래프로 보고 여기에 filter개수 * n^2 행렬 곱하면 => filter * n^2 행렬
        # 4. 여기에 b를 더하고 렐류를 취한 후에
        # 이걸 다시 원래 형태로 바꾸고 넘겨주자 => n * n * filter
        [n, n, filter_cnt] = tf.shape(x).shape.as_list()
        x = tf.reshape(x, (n * n, filter_cnt))
        x = tf.transpose(x)
